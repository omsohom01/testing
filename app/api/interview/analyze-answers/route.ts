...
export async function POST(request: NextRequest) {
  try {
    ...
    // Accepts either [{id, question, type}, ...] and [answer, ...] or [{id, question, type, answer}, ...]
    let qArr = questions;
    let aArr = answers;
    if (!Array.isArray(questions) && Array.isArray(answers)) {
      // If only answers array is provided, try to extract questions from it
      qArr = answers.map((a: any) => ({ id: a.id, question: a.question, type: a.type }));
      aArr = answers.map((a: any) => a.answer);
    }
    if (!Array.isArray(qArr) || !Array.isArray(aArr) || qArr.length !== aArr.length) {
    ...
    const finalScore = Math.round((totalScore / qArr.length) * 10) / 10;
